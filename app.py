...
_SESSION_CLEANUP_INTERVAL = 3600.0
...
def api_pdf_tools_job_thumbnail(job_id: str, page_number: int):
    ...
    if page_count:
        ...
        candidates.append(root / f"page-{page_number:0{digits}d}.png")
    candidates.append(root / f"page-{page_number}.png")

    thumb: Path | None = None
    for candidate in candidates:
        try:
            candidate = _safe_path(candidate, root)
        except ValueError:
            return jsonify({"ok": False, "msg": "Not found"}), 404
        if candidate.exists():
            thumb = candidate
...
def api_pdf_tools_job_image_thumbnail(job_id: str, image_index: int):
    ...
    try:
        thumb = _safe_path(root / f"image-{image_index:0{digits}d}.png", root)
    except ValueError:
        return jsonify({"ok": False, "msg": "Not found"}), 404
    if not thumb.exists():
        return jsonify({"ok": False, "msg": "Thumbnail not ready"}), 404
...
def api_pdf_tools_job_image(job_id: str, image_index: int):
    ...
    paths = pdf_jobs.build_job_paths(job_id)
    try:
        target = _safe_path(paths.input_dir / stored, paths.input_dir)
    except ValueError:
        return jsonify({"ok": False, "msg": "Image not found"}), 404
...
def api_pdf_tools_job_image_preview(job_id: str):
    ...
    paths = pdf_jobs.build_job_paths(job_id)
    try:
        target = _safe_path(paths.output_dir / preview_name, paths.output_dir)
    except ValueError:
...
def api_pdf_tools_job_merge_thumbnail(job_id: str, file_index: int):
    ...
    try:
        thumb = _safe_path(root / f"file-{file_index:02d}.png", root)
    except ValueError:
        return jsonify({"ok": False, "msg": "Not found"}), 404
    if not thumb.exists():
        return jsonify({"ok": False, "msg": "Thumbnail not ready"}), 404
...
def api_pdf_tools_job_view(job_id: str):
    ...
    paths = pdf_jobs.build_job_paths(job_id)
    try:
        target = _safe_path(paths.output_dir / result_filename, paths.output_dir)
    except ValueError:
