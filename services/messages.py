...
def list_inbox(user_id: int, limit: int = 50) -> List[Message]:
    conn = get_app_db()
    rows = conn.execute(
        """
...
def list_sent(user_id: int, limit: int = 50) -> List[Message]:
    conn = get_app_db()
    rows = conn.execute(
        """
...
def get_message(message_id: int, user_id: int) -> Optional[Message]:
    conn = get_app_db()
    row = conn.execute(
        """
